...
void main()
{
    ...
    float lightmap  = texture2D(R5_texture2, pos.xy).r;
    ...
    // R5_FOR_EACH_LIGHT
    {
        ...
        //diffuse += gl_LightSource[0].ambient * lightmap + gl_LightSource[0].diffuse * diffuseFactor;

		// Fake AO. AO shouldn't affect diffuse, but doing so makes the effect more noticeable.
        diffuse += (gl_LightSource[0].ambient + gl_LightSource[0].diffuse * diffuseFactor) * lightmap;
