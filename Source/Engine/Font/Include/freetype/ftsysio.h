...
FT_BEGIN_HEADER
  ...
  *   streamRead :: stream byte read method
  *   streamSeek :: stream seek method
  */
  typedef struct FT_Stream_ClassRec_
  {
    FT_ClassRec          clazz;
    FT_Stream_ReadFunc   streamRead;
    FT_Stream_SeekFunc   streamSeek;
...
#define  FT_STREAM_CLASS__READ(x)  FT_STREAM_CLASS(x)->streamRead
#define  FT_STREAM_CLASS__SEEK(x)  FT_STREAM_CLASS(x)->streamSeek;
...
#define  FT_STREAM_IS_BASED(x)  ( FT_STREAM(x)->base != NULL )
 ...
 /* create new memory-based stream */
  FT_BASE( FT_Error )   ft_streamNew_memory( const FT_Byte*  streamBase,
                                              FT_ULong        streamSize,
                                              FT_Memory       memory,
                                              FT_Stream      *astream );

  FT_BASE( FT_Error )   ft_streamNew_iso( const char*  pathanme,
                                           FT_Memory    memory,
                                           FT_Stream   *astream );
 ...
 /*                                                                 */
  FT_APIVAR( FT_Type )   ft_streamDefault_type;
