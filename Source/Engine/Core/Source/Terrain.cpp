...
uint Terrain::OnDraw (TemporaryStorage& storage, uint group, const ITechnique* tech, void* param, bool insideOut)
{
	if ( mMat != 0 && group  == mMat->GetUID() && mMat->IsVisibleWith(tech) )
	{
		...
		return QuadTree::OnDraw(storage, group, tech, param, insideOut);
	}
	return 0;
}
