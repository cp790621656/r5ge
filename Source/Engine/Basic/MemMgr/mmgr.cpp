...
{
	char	*ptr = strrchr((char*)((void*)&sourceFile), '\\');
	if (ptr) return ptr + 1;
	ptr = strrchr((char*)((void*)&sourceFile), '/');
	if (ptr) return ptr + 1;
	return sourceFile;
}
...
static	const char	*memorySizeString(ulong size)
{
	static	char	str[90];
...
static	void	wipeWithPattern(sAllocUnit *allocUnit, ulong pattern, const unsigned int originalReportedSize = 0)
{
...
bool	&mBreakOnRealloc(void *reportedAddress)
{
...
bool	&mBreakOnDealloc(void *reportedAddress)
{
	// Locate the existing allocation unit
...
void	mBreakOnAllocation(unsigned int count)
{
...
void	mSetOwner(const char *file, const unsigned int line, const char *func)
{
	...
	//
	// mSetOwner(__FILE__, __LINE__, __FUNCTION__) --> object::~object() --> delete
	//
	...
	//
	// mSetOwner(...) -> ~obj1()                          // original call to delete obj1
	//     mSetOwner(...) -> ~obj2()                      // obj1's destructor deletes obj2
	//         mSetOwner(...) -> ~obj3()                  // obj2's destructor deletes obj3
	...
	//
	// Because mSetOwner() just sets up some static variables (below) it's important that each call to mSetOwner() and
	// successive calls to new/delete alternate. However, in this case, three calls to mSetOwner() happen in succession
	...
	// mSetOwner() is never called, and rightfully should not have the proper trace-back information. So if one of the
	...
	// In this case, any call to delete with a NULL will sill call mSetOwner(), which will make mSetOwner() think that
...
void	operator delete(void *reportedAddress)
{
	...
	if (reportedAddress) mDeallocator(sourceFile, sourceLine, sourceFunc, m_alloc_delete, reportedAddress);
...
void	operator delete[](void *reportedAddress)
{
	...
	if (reportedAddress) mDeallocator(sourceFile, sourceLine, sourceFunc, m_alloc_delete_array, reportedAddress);
...
void	*m_allocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int allocationType, const size_t reportedSize)
{
	try
	{
		...
		if (alwaysValidateAll) mValidateAllAllocUnits();
...
void	*mReallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int reallocationType, const size_t reportedSize, void *reportedAddress)
{
	try
	{
		#ifdef TEST_MEMORY_MANAGER
		log("[D] ENTER: mReallocator()");
		#endif
		...
		m_assert(mValidateAllocUnit(au));
...
void	mDeallocator(const char *sourceFile, const unsigned int sourceLine, const char *sourceFunc, const unsigned int deallocationType, const void *reportedAddress)
{
	try
	{
		#ifdef TEST_MEMORY_MANAGER
		log("[D] ENTER: mDeallocator()");
		#endif
		...
		// We should only ever get here with a 0 pointer if they try to do so with a call to free() (delete[] and delete will
...
bool	mValidateAddress(const void *reportedAddress)
{
...
bool	mValidateAllocUnit(const sAllocUnit *allocUnit)
{
...
bool	mValidateAllAllocUnits()
{
	...
	for (unsigned int i = 0; i < hashSize; i++)
	{
		...
		{
			allocCount++;
			if (!mValidateAllocUnit(ptr)) errors++;
			ptr = ptr->next;
		}
	}
	...
	if (errors) log("[!] While validting all allocation units, %u allocation unit(s) were found to have problems", errors);
...
unsigned int	mCalcUnused(const sAllocUnit *allocUnit)
{
	const ulong	*ptr = reinterpret_cast<const ulong *>(allocUnit->reportedAddress);
...
void	mDumpAllocUnit(const sAllocUnit *allocUnit, const char *prefix)
{
	...
	log("[I] %sAllocation number : %u",          prefix, allocUnit->allocationNumber);
}

// ---------------------------------------------------------------------------------------------------------------------------------

void	mDumpMemoryReport(const char *filename, const bool overwrite)
{
	// Open the report file
