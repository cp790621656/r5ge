...
// mCodeptr member is *always* called, regardless of the values of the other
...
struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE + 3*sizeof(int) >
{
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		union {
			...
			// is internally defined as:
			struct {
				GenericMemFuncType mFuncaddress; // points to the actual member function
				int delta;		// #bytes to be added to the 'this' pointer
...
protected: 
	...
	detail::GenericClass *mPthis;
	GenericMemFuncType mPFunction;

#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
	typedef void (*GenericFuncPtr)(); // arbitrary code pointer
	GenericFuncPtr mPStaticFunction;
#endif

public:
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
	DelegateMemento() : mPthis(0), mPFunction(0), mPStaticFunction(0) {};
	void clear() {
		mPthis=0; mPFunction=0; mPStaticFunction=0;
	}
#else
	DelegateMemento() : mPthis(0), mPFunction(0) {};
	void clear() {	mPthis=0; mPFunction=0;	}
#endif
public:
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
	inline bool IsEqual (const DelegateMemento &x) const{
	    // We have to cope with the static function pointers as a special case
		if (mPFunction!=x.mPFunction) return false;
		// the static function ptrs must either both be equal, or both be 0.
		if (mPStaticFunction!=x.mPStaticFunction) return false;
		if (mPStaticFunction!=0) return mPthis==x.mPthis;
		else return true;
	}
#else // Evil Method
	inline bool IsEqual (const DelegateMemento &x) const{
		return mPthis==x.mPthis && mPFunction==x.mPFunction;
	}
#endif
...
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
		if (mPStaticFunction !=0 || right.mPStaticFunction!=0) 
				return mPStaticFunction < right.mPStaticFunction;
#endif
		if (mPthis !=right.mPthis) return mPthis < right.mPthis;
	// There are no ordering operators for member function pointers, 
	// but we can fake one by comparing each byte. The resulting ordering is
	// arbitrary (and compiler-dependent), but it permits storage in ordered STL containers.
		return memcmp(&mPFunction, &right.mPFunction, sizeof(mPFunction)) < 0;

	}
	// BUGFIX (Mar 2005):
	// We can't just compare mPFunction because on Metrowerks,
	// mPFunction can be zero even if the delegate is not empty!
	inline bool operator ! () const		// Is it bound to anything?
	{ return mPthis==0 && mPFunction==0; }
...
public:
	...
	inline void bindmemfunc(X *pthis, XMemFunc function_to_bind ) {
		mPthis = SimplifyMemFunc< sizeof(function_to_bind) >
			::Convert(pthis, function_to_bind, mPFunction);
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
		mPStaticFunction = 0;
#endif
	}
	...
	inline void bindconstmemfunc(const X *pthis, XMemFunc function_to_bind) {
		mPthis= SimplifyMemFunc< sizeof(function_to_bind) >
			::Convert(const_cast<X*>(pthis), function_to_bind, mPFunction);
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK)
		mPStaticFunction = 0;
#endif
	}
	...
	inline GenericClass *GetClosureThis() const { return mPthis; }
	inline GenericMemFunc GetClosureMemPtr() const { return reinterpret_cast<GenericMemFunc>(mPFunction); }
...
public:
	...
	// We may need to convert the mPthis pointers, so that
	// they remain as self-references.
	template< class DerivedClass >
	inline void CopyFrom (DerivedClass *pParent, const DelegateMemento &x) {
		SetMementoFrom(x);
		if (mPStaticFunction!=0) {
			// transform self references...
			mPthis=reinterpret_cast<GenericClass *>(pParent);
		}
	}
	...
	inline void bindstaticfunc(DerivedClass *pParent, ParentInvokerSig static_function_invoker, 
		...
		mPStaticFunction=reinterpret_cast<GenericFuncPtr>(function_to_bind);
	}
	inline UnvoidStaticFuncPtr GetStaticFunction() const { 
		return reinterpret_cast<UnvoidStaticFuncPtr>(mPStaticFunction); 
	}
#else
...
// support static_cast between void * and function pointers.
	...
	inline void bindstaticfunc(DerivedClass *pParent, ParentInvokerSig static_function_invoker, 
		...
		mPthis = horrible_cast<GenericClass *>(function_to_bind);
		// MSVC, SunC++ and DMC accept the following (non-standard) code:
//		mPthis = static_cast<GenericClass *>(static_cast<void *>(function_to_bind));
		// BCC32, Comeau and DMC accept this method. MSVC7.1 needs __int64 instead of long
//		mPthis = reinterpret_cast<GenericClass *>(reinterpret_cast<long>(function_to_bind));
	}
	// ******** EVIL, EVIL CODE! *******
...
public:
	operator unspecified_bool_type() const {
        return empty()? 0: &SafeBoolStruct::mNonzero;
    }
